...
public class RtpMediaExtractor implements RtpSessionDataListener, MediaExtractor {
    ...
    public void dataPacketReceived(RtpSession session, RtpParticipantInfo participant, DataPacket packet) {
        ...
        if (RtpMediaDecoder.DEBUGGING) {
            android.util.Log.e("STREAMER", debugging);
        }
        ...
        switch (h264Packet.h264NalType) {
            case FULL:
                if (RtpMediaDecoder.DEBUGGING) {
                    android.util.Log.i("STREAMER", "NAL: full packet");
                }
            ...
            case FUA:
                if (RtpMediaDecoder.DEBUGGING) {
                    android.util.Log.i("STREAMER", "NAL: FU-A fragment");
                }

                // Do we have a clean start of a frame?
                if (h264Packet.isStart()) {
                    if (RtpMediaDecoder.DEBUGGING) {
                        android.util.Log.i("STREAMER", "FU-A start found. Starting new frame");
                    }
                ...
                if (currentFrame != null) {
                    ...
                    if (packet.getTimestamp() != currentFrame.getRtpTimestamp()) {
                        if (RtpMediaDecoder.DEBUGGING) {
                            android.util.Log.w("STREAMER", "Non-consecutive timestamp found");
                        }
                    ...
                    if (!currentFrameHasError) {
                        ...
                        android.util.Log.i("STREAMER", "Dropping frame");
                    }

                    if (h264Packet.isEnd()) {
                        if (RtpMediaDecoder.DEBUGGING) {
                            android.util.Log.i("STREAMER", "FU-A end found. Sending frame!");
                        }
                        try {
                            sendFrame();
                        } catch (Throwable t) {
            ...
            case STAPA:
                if (RtpMediaDecoder.DEBUGGING) {
                    android.util.Log.i("STREAMER", "NAL: STAP-A");
                }
                ...
                break;
            case UNKNOWN:
                android.util.Log.w("STREAMER", "NAL: Unimplemented unit type: " + h264Packet.getNalType());
