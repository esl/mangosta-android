...
public class RtpMediaDecoder implements Decoder, SurfaceHolder.Callback {
    ...
    public static final int SURFACE_WIDTH = 1920;
    ...
    public RtpMediaDecoder(SurfaceView surfaceView, Properties properties) {
        ...
        android.util.Log.i("STREAMER", "RtpMediaDecoder started with params (" + DEBUGGING + "," + bufferType + "," + useNio + "," + receiveBufferSize + ")");
    ...
    public void decodeFrame(BufferedSample decodeBuffer) throws Exception {
        if (DEBUGGING) {
            ...
            android.util.Log.i("STREAMER", decodeBuffer.toString());
        }
        ...
        switch (outIndex) {
            case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
                if (DEBUGGING) {
                    android.util.Log.i("STREAMER", "The output buffers have changed.");
                }
                outputBuffers = decoder.getOutputBuffers();
                break;
            case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                if (DEBUGGING) {
                    android.util.Log.i("STREAMER", "New format " + decoder.getOutputFormat());
                }
                break;
            case MediaCodec.INFO_TRY_AGAIN_LATER:
                if (DEBUGGING) {
                    android.util.Log.i("STREAMER", "Call to dequeueOutputBuffer timed out.");
                }
                break;
            default:
                if (DEBUGGING) {
                    ByteBuffer buffer = outputBuffers[outIndex];
                    android.util.Log.i("STREAMER", "We can't use this buffer but render it due to the API limit, " + buffer + " outIndex " + outIndex);
                }
        ...
        if (((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) && DEBUGGING) {
            android.util.Log.i("STREAMER", "All decoded frames have been rendered");
        }
    }
...
//        surfaceView.setLayoutParams(layoutParams);
    ...
    private class PlayerThread extends Thread {
        ...
        public void run() {
            ...
            if (decoder == null) {
                android.util.Log.i("STREAMER", "Can't find video info!");
                return;
            }
    ...
    private class RTPClientThread extends Thread {
        ...
        public void run() {
            ...
            android.util.Log.i("STREAMER", "RTP Session created");
